...
- Release the preflight camera preview before FFmpeg acquires the camera.
...
- Command mode is a small top-right panel that **always opens in the same place and never moves while open** (`WindowPlacement.PlaceTopRight`). An earlier pointer keep-away behaviour was removed for predictability — do not reintroduce it. Clicking outside deliberately does not dismiss it.
...
- `DoubleCircleGestureDetector` is permissive about gesture size and speed and strict only about shape (angular travel, directionality, radius consistency). Loosen size/speed if asked; do not loosen the shape checks, which are what stop ordinary mouse movement from opening command mode.
