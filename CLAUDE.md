...
- `build-native.ps1` publishes with `-p:PublishTrimmed=false -p:PublishReadyToRun=false`, overriding the csproj. Nothing that ships is trimmed, and editing the csproj alone changes nothing about the artifact.
