...
- The main window, annotation workspace, command mode, recording preflight, HUD, and receipts all receive their backdrop through `ThemeCoordinator`; XAML windows never declare a second backdrop. On supported systems, each eligible window owns one custom `SystemBackdrop` backed by `DesktopAcrylicController`, allowing the five transparency levels to tune the compositor's whole-window tint and luminosity live. The built-in `DesktopAcrylicBackdrop` remains only as the unsupported-controller compatibility path.
- Settings exposes five live, autosaved glass-transparency presets: Very transparent, More transparent, Balanced, More solid, and Very solid. Balanced preserves the established dark panel/raised tints (`0.48`/`0.62`) and light tints (`0.54`/`0.68`). Each step changes Acrylic tint, Acrylic luminosity, and semantic-layer opacity together, with deliberately dramatic extremes so the difference is unmistakable on the two Library panes. The choice tunes CursorPocket's material layers, not Windows' blur availability or adaptive fallback policy. Invalid or missing values normalize to Balanced.
...
- **Liquid glass.** A configurable Desktop Acrylic controller blurs the live desktop behind command mode and follows the same five transparency levels as the Library window. The transparent shell exposes that desktop-sampling material directly; a neutral inner rim and one top-edge catch-light give the pane thickness, and the green hairline remains state rather than decoration. The frozen desktop snapshot it replaced now belongs to region selection alone.
...
- Selection is carried by fill, not by a green wash. Filters are one segmented control with a live count per filter. At wide widths that control shares the action toolbar so the list and preview start on the same edge; at narrower widths it takes a full row below the actions rather than clipping or hiding capture types.
...
- Exactly four approved raster masters live in `assets/brand/` and `assets/brand/imagery/`; the small final PNG/ICO deliverable set lives in `assets/brand/export/` and is regenerated without redrawing the masters using `python tools/make_brand_assets.py`. Do not retain superseded logo iterations, alternate application-state marks, or per-size PNG intermediates.
...
- all five glass levels apply live to the compositor backdrop of every eligible open window and to its inner chrome, visibly change Library and Command Center, persist atomically, and leave high contrast and the dense recording HUD unchanged;
